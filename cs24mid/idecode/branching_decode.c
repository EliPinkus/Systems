...
void fetch_and_decode(InstructionStore *is, Decode *d, ProgramCounter *pc) {
    ...
    busdata_t operation = (instr_byte >> ALUOP_POS) & ALUOP_MASK;
    ...
    switch (operation) {
        ...
        case OP_SHR:
            ...
            src1_addr = ((instr_byte >> REG_B_POS) & REG_MASK);
            src2_addr = ((instr_byte >> REG_B_POS) & REG_MASK);
            dst_addr = ((instr_byte >> REG_B_POS) & REG_MASK);
            break;

        /* 2-argument ALU OP -> 2 byte */
        case OP_MOV:
        case OP_ADD:
        case OP_SUB:
        case OP_AND:
        ...
        case OP_XOR:
            dst_write = WRITE_REG;
            src1_isreg = ((instr_byte >> TWO_REG_IS_REG_POS) & IS_REG_MASK);
            src2_addr = ((instr_byte >> REG_B_POS) & REG_MASK);
            dst_addr = ((instr_byte >> REG_B_POS) & REG_MASK);
            ...
            /* save either REG A or given constant */
            if (src1_isreg) {
                src1_addr = ((instr_byte >> TWO_REG_REG_A_POS) & REG_MASK);
            }
            else {
        ...
        case OP_BRZ:
        case OP_BNZ:
            branch_addr = (instr_byte >> DEST_ADDR_POS) & DEST_ADDR_MASK;
            break;
        default:
