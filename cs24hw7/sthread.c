...
void sthread_start(int timer) {
    /* we want to resume later so we lock before we switch so process is not
     * interrupted while saving state to stack
     */
    __sthread_lock();
    ...
    /*
     * We want to allow access to other threads so we unlock.
     */
    __sthread_unlock();
}
...
void __sthread_finish(void) {
    /*
     * we lock before switching state of thread such that process is not
     * interrupted while saving state
     */
     __sthread_lock();
    ...
    /*
     * We want to allow access to the next thread so we unlock!
     */
    __sthread_unlock();
}
...
void sthread_block() {
    ...
    /*
     * We want this to run atomically so we lock yo!
     */
    __sthread_lock();
