...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    *offset = get_offset_in_block(p_cache, address);
    ...
    /* use bitmask to extract set and tag */
    uint32_t setmask = (p_cache->num_sets - 1) << p_cache->block_offset_bits;
    *set = (address & setmask) >> p_cache->block_offset_bits;

    /* tag size = total block - offset - sets addrress */
    uint32_t tsize = p_cache->block_size - p_cache->block_offset_bits -
                                        p_cache->sets_addr_bits;
    uint32_t tagmask = ((0x1 << tsize) - 1) << (p_cache->block_size - tsize);
    *tag = (address & tagmask) >> (p_cache->block_size - tsize);
}
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
    return (tag << (p_cache->block_offset_bits + p_cache->sets_addr_bits))
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    for (int32_t i = 0; i < p_set->num_lines; i++) {
        if (p_set->cache_lines[i].valid && p_set->cache_lines[i].tag ==tag ) {
...
#else
    /* evict least recently used line via simple search */
    uint64_t oldest = p_set->cache_lines[0].last_access;
    i_victim = 0;

    for (int32_t i = 0; i < p_set->num_lines; i++) {
        /* return right away if invalid line */
        if (!p_set->cache_lines[i].valid) {
            return &(p_set->cache_lines[i]);
        }

        if (p_set->cache_lines[i].last_access < oldest) {
            oldest = p_set->cache_lines[i].last_access;
            i_victim = i;
        }
    }
