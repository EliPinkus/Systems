...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    if (context != NULL) {
        /* Queue thread if ready or blocked state */
        ...
        /* ready thread and cue it if state is running */
    ...
    /* run first ready thread */
    current = queue_take(&ready_queue);

    if (current == NULL) {

        if (blocked_queue.head != NULL) {
            printf("Error: there are blocked threads");
            exit(1);
        }
        else {
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    /* allocate memory to hold context and thread */
    void * stackptr = malloc(DEFAULT_STACKSIZE);
    if (stackptr == NULL) {
        printf("ERROR:  couldn't allocate %u bytes!\n",
               (unsigned int) DEFAULT_STACKSIZE);
        exit(1);
    }
    ...
    if (thread == NULL) {
        printf("ERROR:  couldn't allocate %u bytes!\n",
               (unsigned int) sizeof(Thread));
        exit(1);
    }

    thread->context = __sthread_initialize_context((char *) (stackptr) +
                                            DEFAULT_STACKSIZE, f, arg);

    thread->state = ThreadReady;
    thread->memory = stackptr;
