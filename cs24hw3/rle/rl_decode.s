...
find_space_loop:
        add     (%rdi, %rcx), %bl         # Add in the count, then move, sets CF
        jnc     find_space_continue       # skip next line if no carry flag
        add     $256, %ebx                # account for carry if necessary

find_space_continue:
      add     $2, %ecx                    # inc by 2 to skip char value
      cmp     %esi, %ecx
      jl      find_space_loop
...
find_space_done:
        ...
        # push other caller-save registers to stack to retrieve later
        push    %rdi
        push    %rsi
        push    %rdx
        ...
        # pop caller-save register values from earlier back into place
        pop     %rdx
        pop     %rsi
        pop     %rdi
...
write_loop:
        mov     %bl, (%rax, %r10)
        inc     %r10              # increment %r10 s.t. chars placed properly
        dec     %bh
        jnz     write_loop
