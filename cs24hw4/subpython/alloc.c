...
void marker(const char *n, Reference ref);
...
/*
 * a helper function to perform the marking step of mark and sweep
 */
void marker(const char *n, Reference ref) {

    // n is unused arg (placeholder)

    if (ref == NULL_REF) {
        return;
    }

    ValueType type = (deref(ref))->type;
    Value *val = (deref(ref));

    // Check Value type
    if (type == VAL_FLOAT || type == VAL_STRING) {
        val->mark = true;
    }
    else if (type == VAL_LIST_NODE) {
        ...
        if (!lval->mark) {
            ...
            /* make recursive calls to account for nesting */
            lval->mark = true;
            marker(n, lval->list_node.value);
            marker(n, lval->list_node.next);
        }
    }
    else if (type == VAL_DICT_NODE) {
        ...
        if (!dval->mark) {
            ...
            /* make recursive calls to account for nesting */
            dval->mark = true;
            marker(n, dval->dict_node.value);
            marker(n, dval->dict_node.key);
            marker(n, dval->dict_node.next);
        }
    }
...
/*
 * Garbage collector for subpython interpreter
...
int collect_garbage(void) {
    ...
    while (i < freeptr) {
        ...
        /* otherwise rearrange data pool */
        else {
            val->mark = false;
            memmove((void *) dest, (const void *) i,
                                                sizeof(Value) + size);
            ref_table[ref] = (Value *) dest;
            dest += sizeof(Value) + size;
        }
        i += sizeof(Value) + size;
    }
